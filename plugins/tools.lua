...
--Begin Tools.lua :)
local SUDO = 321681775 -- put Your ID here! <===
	...
	if msg.to.type == 'channel' or msg.to.type == 'chat' then
		if matches[1] == 'charge' and matches[2] and not matches[3] and is_sudo(msg) then
			if tonumber(matches[2]) > 0 and tonumber(matches[2]) < 1001 then
				...
				if lang then
					tdcli.sendMessage(msg.to.id, msg.id_, 1, 'Robot set successfully\nLong active in the group robot '..matches[2]..' The other day set...', 1, 'md')
					tdcli.sendMessage(SUDO, 0, 1, 'Robot Group '..matches[2]..' For`'..msg.to.id..'` Day extended.', 1, 'md')
				else
					tdcli.sendMessage(msg.to.id, msg.id_, 1, 'Robot set successfully\nLong active in the group robot '..matches[2]..' The other day set...', 1, 'md')
					tdcli.sendMessage(SUDO, 0, 1, 'Robot Group '..matches[2]..' For`'..msg.to.id..'` Day extended.', 1, 'md')
				end
